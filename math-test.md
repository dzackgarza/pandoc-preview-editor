...
tags:
  ...
  - type/resource
aliases: [The cotangent complex and derived de Rham cohomology]
created: unknown
...
Basic affine objects in AG: commutative rings, replace with [simplicial commutative rings](simplicial commutative rings) which we'll use as a base diagram.
Later: [geometric derived stacks](geometric derived stacks).
...
For $X$ a smooth and proper $k\dash$scheme, the [Picard stack](Picard stack) 
\[
\underline{\Pic}_{X/k}
\]

is an [[Deligne-Mumford stack|Deligne-Mumford stacks]].
...
:::{.remark}
Note that the [mapping stack](mapping stack),
\[  
...
$K(\GG_m, n)$ is a [higher stack](higher stack), thought of as a sheaf taking values in $n\dash$truncated spaces $\mathcal{S}_{\leq n}$, i.e. a space where when basing at any point, there are no homotopy groups above degree $n$:
\[  
...
:::{.remark}
Note that these are all built from [affine scheme](affine%20scheme) with a few acceptable moves.
:::
...
Mapping into this gives the [Picard groupoid](Picard groupoid) of a scheme.
These are higher [geometric stacks](geometric stacks) that still have some "smoothness" properties.
:::
...
Note that the category of [[enriched category]] in topological spaces.
...
:::{.remark}
Why [[derived algebraic geometry|derive]] things?
...
An example is $\mathcal{M}_\phi$, the moduli of objects in some [differential graded category](differential graded category) $\mathcal{C}$.
:::

## Simplicial Rings

We need to agree on what the local affine modules will look like. 
For our purposes, they'll be [simplicial commutative rings](simplicial commutative rings).
Consider the derived category  $\mathcal{C} \da \der(\ZZ)_{\geq 0}$ and its connective[^def_connective]
objects, which are chain complexes $C_\wait$ where $H_{< 0}(C_\wait) = 0$.
There is a [derived tensor product](derived tensor product)  $\wait\tensor^\LL \wait$ which makes $\mathcal{C}$ into a [symmetric monoidal category](Symmetric monoidal category.md).
...
$\ssets \homotopic \Top$: this is not an equivalence of categories, but rather they have *equivalent homotopy theories* (see [[Weak equivalence|weak equivalence]] [^def_weak_equivalence] ) on each side.
...
Considering $\Delta^\wait_{\Top}$, this is a [cosimplicial object](cosimplicial object) in $\Top$.
...
Note that this is just composing functors $\Delta\op \to\Sets$ and $\Set \to \mods{\ZZ}$.
We can use this to create a chain complex $\complex{C} (\ZZ[\text{Sing}(X)])$, and as expected, we get the singular homology:
\[
...
This was studied by Quillen, and an impetus for [model categories](model categories).
...
there is an [[adjoint (categorical)|adjunction]] from sets to commutative $k\dash$algebras
...
Any time such an adjunction exists, given an $R\in \mathrm{CAlg}_k$ we can construct a [simplicial resolution](simplicial resolution.md) $S^\wait$ and a map $S^\wait \to R$.
This resolution has the following structure:
\[  
S^0 = k[R], \,\, \text{the free commutative algebra on }R
.\]
Using the unit and counit maps of the adjunction, one obtains a canonical [simplicial object](simplicial object.md),
\begin{tikzcd}
  k[R]
...
So we've taken an arbitrary $k\dash$algebra and replaced it with a [simplicial k-algebra](simplicial k-algebra) which is given by polynomial rings in each degree, typically in infinitely many variables, which has the same homology.
This is the analog of a [projective resolution](projective resolution).
...
given by formally adjoining [filtered colimits](filtered colimits).
...
So starting with a functor defined on finitely generated polynomial rings, i.e. affine spaces, we get a simplicial [derived functors](derived functors).
For $R\in \mathrm{CAlg}_k$, using $S^\wait \mapsvia{\sim} R$, we can apply $F$ level-wise to get a new [simplicial object.md) $F(S^\wait](simplicial object.md) $F(S^/wait) \in \mathcal{C}$.
...
\]
So we can promote functors on polynomial rings to functors on [simplicial commutative rings](simplicial commutative rings).
...
& 
\end{tikzcd}
Take [Kahler differentials](Kahler differentials) degree-wise to get 
$\Omega^1_{S^\wait/k}$.
...
We can now use [[cotangent complex]] of $R/k$** as
\[  
...
There is a natural [[Enriched category]] in topological spaces, so we'll write $\Map$ for homs with their topological structure.
...
where the latter is in the category of simplicial commutative $k\dash$algebras with a *fixed* map to $S$, a bit like a [comma category](comma category).
This makes sense because $R\to S$ is a fixed map, and $S\oplus M$ has a projection to $S$, i.e. this is a square zero extension.

:::{.remark}
The point of [simplicial commutative rings](simplicial commutative rings) is that this extension still makes sense, even when $M$ is a chain complex.
It's still possible to make this into a simplicial commutative ring.
This may look familiar from the exercises in Hartshorne, since it resembles the definition of [Kahler differentials](Kahler differentials).
:::
...
So the RHS [comma category](comma category) mapping space in the previous equivalence is denoted the "space of derivations of $R$ and $M$" (whatever that means).

:::


:::{.exercise title="?"}
You can base-change the [[cofiber sequence]]
...
,\]
which takes a polynomial ring $R$ to its algebraic [de Rham complex](de Rham complex).
...
Then $\dR_{R/k} \cong k$ by the [Poincaré lemma](Poincaré lemma), just the cohomology of affine space.
...
[^what_its_like]: 
This is like defining the [p-adic integers](p-adic integers.md) as the limit of $\ZZ/p^n\ZZ$.
