...
async function main() {
  ...
  // Use a string-based evaluate to avoid transpiler issues
  const measures = await page.evaluate(new Function(`
  ...
  if (measures.xtermViewport && measures.terminalPane) {
    var fill = (measures.xtermViewport.h / measures.terminalPane.h * 100).toFixed(1);
    console.log('\nFill ratio: ' + fill + '%');
  }
  if (measures.xterm && measures.terminalPane) {
    console.log('Gaps: top=' + (measures.xterm.t - measures.terminalPane.t).toFixed(0) + 'px, bottom=' + (measures.terminalPane.b - measures.xterm.b).toFixed(0) + 'px');
  }
