...
test.describe('plugin API', () => {
  savedFileTest(
    ...
    async ({ appPage }) => {
      const data = await invoke(appPage, 'list_plugins', {});
      ...
      const exportHtml = (data.plugins as Array<Record<string, unknown>>).find(
        (p) => p.id === 'export-html',
      );
  ...
  test('plugin rejects run without a saved file path', async ({ appPage }) => {
    try {
      await invoke(appPage, 'run_plugin', {
        id: 'export-html',
  ...
  savedFileTest(
    ...
    async ({ appPage, testEnv }) => {
      ...
      const result = await invoke(appPage, 'run_plugin', {
        id: 'export-html',
        path: documentPath,
