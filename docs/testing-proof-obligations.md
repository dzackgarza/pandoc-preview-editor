...
It does **not** prescribe implementation details or test helpers. It defines what the suite should **prove** about real owned behavior.
...
This document is the proof-obligation specification. It does not track migrated-file inventory and does not report whether the app currently passes the suite.
...
A failing test is not automatically evidence that the app is wrong. Before editing app code, assertions, fixtures, helpers, timeouts, or adapter calls, classify the first incorrect boundary:
...
Repeated hacking is a failure mode. After two failed fix attempts on the same test, stop editing and review the causal note. Do not get to green by increasing timeouts, adding retries, narrowing assertions, switching to mocked IPC, deleting/skipping the test, adding local duplicate helpers, swallowing diagnostics, or changing expectations to match current broken behavior.
...
6.  **Route-interception error tests**
    - Old tests that used `page.route(...)` to fake 500s are not acceptable as primary proofs. If the failure mode matters, reproduce it with a real failing renderer, real conflicting file, real missing tool, or real bad config.

## Proof obligations the recovered suite should cover

Each item below is phrased as a repository-owned guarantee. Unless noted otherwise, each should be proved with real Playwright interaction against the real Tauri app plus real filesystem/process boundaries.
...
1.  **Unsaved work survives reload and restart**
    - Prove both:
...
2.  **Initial state reports real file identity correctly**
...
1.  **The preview is a faithful live view of real renderer output**
...
3.  **Renderer failures surface actionable diagnostics and recover cleanly**
...
1.  **Preferences reflect, edit, and persist real config**
...
2.  **Config path restrictions are enforced at the user boundary**
...
3.  **Startup config discovery is real**
...
1.  **Plugins run against the app-owned file path, not a temp fiction**
...
2.  **Plugin save gate is enforced**
...
3.  **Plugin success and failure surface correct UI state**
...
1.  **TikZ is rendered server-side to static SVG**
...
2.  **TikZ auxiliary file resolution works in real documents**
...
1.  **Quick Open drives a real launcher pipeline**
...
2.  **Citation insertion uses the citation boundary, not the save boundary**
...
3.  **Status chrome is trustworthy during a real editing session**
...
The migrated suite must model these proofs through **real Playwright user workflows against the live Tauri app**, with mocks treated as a last resort rather than the default testing strategy. Whether the app satisfies those proofs is a later pass/fail question.
